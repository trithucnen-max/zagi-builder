...
- **Bảo toàn 100% định mức cài đặt:** Loại bỏ hoàn toàn thuật toán tự ý bóp nghẹt giảm `scanDailyLimit` xuống con số thực tế vừa quét (13, 20).
- **Phân loại ngắt nhịp chuẩn xác:**
  - *Chưa đạt định mức ngày (`todayCount < scanDailyLimit`):* Lỗi `-216` được định tuyến chuẩn xác là Rate Limit Khung Giờ $\rightarrow$ Chuyển sang trạng thái `⏳ Dừng 1h (Mã -216)` (Cooldown đúng 60 phút). Hết 60 phút hệ thống tự động thức dậy quét tiếp các số còn lại trong ngày.
  - *Đã hoàn thành định mức ngày (`todayCount >= scanDailyLimit`):* Chuyển sang `🌙 Dừng Ngày (00:00)`.
...
- **Xóa bỏ Modal & Badge Quy tắc:** Gỡ bỏ toàn bộ popup lựa chọn 3 phương thức (*Phân tán*, *Gom về 1 nick*, *Đồng bộ tất cả*) và nút `🟢 Quy tắc: Phân tán ⚡ Đổi` ở thanh thông số Lô.
- **Chuẩn hóa luồng gán dữ liệu:** Nick nào quét được SĐT nào thì lưu UID/profile và gán nhãn thuộc về chính nick đó.
...
- **Tự động chuyển đổi sang Single Mode (`findUser`):** Khi Zalo chặn quét gộp (Bulk), hệ thống tự động chuyển tài khoản sang chế độ Single Mode kèm khoảng nghỉ an toàn (`1.5s - 3s/lần`), tiếp tục quét thông suốt toàn bộ các số hợp lệ phía sau số lỗi.
...
### ⏳ Cơ Chế Smart Cooldown 3 Phút & Ngưỡng Lỗi Liên Tiếp 3 Lần
...
- **Rollback về pending & Cooldown 3 phút (`accountCooldownUntil`):** Khi gặp 1-2 lần lỗi đơn lẻ, hệ thống tự động rollback SĐT về `pending` và đưa nick vào trạng thái nghỉ 3 phút để hạ nhiệt.
...
- **`🔄 Đổi Nick quét tiếp`:** Danh sách các tài khoản Zalo đang kết nối ổn định để bấm 1 chạm chuyển toàn bộ các số còn lại sang nick mới và quét tiếp ngay lập tức.
