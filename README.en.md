...
> **Latest version: v3.2.1** — [View all versions](#changelog)
...
<details open>
<summary><strong>v3.2.1</strong> — 2026-09-24 · <em>🟢 Current version</em></summary>

### 🚀 Key Highlights & Bug Fixes

- 🎯 **Standardize Multi-Block Campaign Send Mode (`CampaignDetail.tsx`, `CRMQueueService.ts`)**:
  - Fixed `modeText` display badge in campaign detail: accurately displays `"Send all (N blocks)"` when campaign send mode is set to `'all'`.
  - Upgraded `CRMQueueService.ts` queue processing to sequentially deliver all message blocks and images to each recipient (for both friends and strangers) without clamp.
- 🏷️ **Fix SQLite Parameter Inversion for Local Labels (`DatabaseService.ts`)**:
  - Corrected reversed parameter order of `contact_id` and `label_id` in contact transfer (`transferContactBetweenAccounts`) and contact merge (`mergeContactsToAccount`), ensuring seamless local label retention across accounts.
- ⚡ **Eliminate Race Conditions in Contact Label Toggling (`CRMContactDetailPanel.tsx`)**:
  - Implemented sequential `for...of` async execution with `localLabelTogglingRef` mutex lock, preventing label loss or state overwrite during multi-selection.
