...
> **Phiên bản mới nhất: v27.2.11** — [Xem tất cả phiên bản](#changelog)
...
│   └─ → Tải: Zagi v27.2.11 Window.exe  ✅
│
├─ 💻 Surface Pro 7, Surface Laptop 1-4, Surface Go 1-2, Surface Book
│   └─ → Tải: Zagi v27.2.11 Window.exe  ✅  (chip Intel, chạy ok)
│
└─ 💻 Surface Pro X, Surface Pro 9 (5G), Surface Pro 10, Surface Pro 11,
       Surface Laptop 7 (chip Snapdragon / ARM64)
    └─ → Tải: Zagi v27.2.11 Surface.exe  ✅ (bản native ARM64)
```

> 💡 **Cách kiểm tra chip máy Surface:** Vào `Settings → System → About`, xem mục **Processor**:
> - Có chữ `Intel` hoặc `AMD` → dùng bản `Zagi v27.2.11 Window.exe`
> - Có chữ `Snapdragon` hoặc `ARM` → dùng bản `Zagi v27.2.11 Surface.exe`
...
│   └─ → Tải: Zagi v27.2.11 MacOS M1+ arm64.dmg  ✅
│
└─ 🍎 MacBook, iMac, Mac mini chip Intel (2019 trở về trước)
    └─ → Tải: Zagi v27.2.11 MacOS Intel.dmg  ✅
```

> 💡 **Cách kiểm tra:** Click logo Apple → **About This Mac** → xem mục **Chip** hoặc **Processor**:
> - Có chữ `Apple M1/M2/M3/M4` → bản `Zagi v27.2.11 MacOS M1+ arm64.dmg`
> - Có chữ `Intel` → bản `Zagi v27.2.11 MacOS Intel.dmg`
...
└─ Fedora, Arch, openSUSE hoặc bất kỳ distro nào
    → Tải .AppImage  ✅
    chmod +x "Zagi v27.2.11 Linux.AppImage" && ./"Zagi v27.2.11 Linux.AppImage"
```
...
|---|---|---|
| PC/Laptop Windows (Intel/AMD) | `Zagi v27.2.11 Window.exe` | Phổ biến nhất |
| Surface Pro 7 trở xuống | `Zagi v27.2.11 Window.exe` | Chip Intel |
| Surface Pro X, 9 5G, 10, 11, Laptop 7 | `Zagi v27.2.11 Surface.exe` | 🆕 Chip ARM64 |
| MacBook M1/M2/M3/M4 | `Zagi v27.2.11 MacOS M1+ arm64.dmg` | Apple Silicon |
| MacBook/iMac Intel | `Zagi v27.2.11 MacOS Intel.dmg` | Intel x64 |
| Ubuntu/Debian Linux | `Zagi v27.2.11 Linux Debian.deb` | Cài như package |
| Fedora/Arch/Linux khác | `Zagi v27.2.11 Linux.AppImage` | Chạy mọi distro |
...
### 🍎 macOS
* ⚠️ **Lưu ý:** Kể từ phiên bản v27.2.11, để phát hành nhanh chóng, bước ký số (Code Signing) tạm thời được bỏ qua. Khi chạy ứng dụng lần đầu, bạn sẽ gặp cảnh báo bảo mật từ Gatekeeper (*"Zagi is damaged and can't be opened"* hoặc *"unidentified developer"*).
