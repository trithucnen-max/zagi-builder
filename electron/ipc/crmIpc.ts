...
export function registerCRMIpc(): void {
    ...
    ipcHandle('crm:createPhoneScanBatch', async (_e, { name, assignedAccountId, targetAccountId, contactAssignmentMode, autoTagIds, dailyLimit, hourlyLimit, priority, status, scheduledTime, skipCrmExisting, autoWorkflowId, updateZaloAlias, phones }: any) => {
        try {
            const db = DatabaseService.getInstance();
    ...
    ipcHandle('crm:convertScanToCampaign', async (_e, {
        batchId,
        zaloId,
        mode,
        existingCampaignId,
        ...
        try {
            const db = DatabaseService.getInstance();
            const sFilter = statusFilter || 'found';
            const sql = sFilter === 'all'
                ? `SELECT zalo_uid, zalo_name, zalo_avatar, phone FROM phone_scan_items WHERE batch_id = ? AND zalo_uid IS NOT NULL AND zalo_uid != ''`
                : `SELECT zalo_uid, zalo_name, zalo_avatar, phone FROM phone_scan_items WHERE batch_id = ? AND status = 'found' AND zalo_uid IS NOT NULL AND zalo_uid != ''`;

            const items = db.query<any>(sql, [batchId]);
            if (!items || items.length === 0) {
                return { success: false, error: 'Không tìm thấy kết quả quét SĐT hợp lệ nào (có Zalo UID) để chuyển sang chiến dịch' };
            }

            const contacts = items.map(item => ({
                contactId: item.zalo_uid,
                displayName: item.zalo_name || item.phone,
                avatar: item.zalo_avatar || '',
                phone: item.phone || ''
            }));

            let targetCampaignId = existingCampaignId;

            if (mode === 'new') {
                if (!newCampaign || !newCampaign.name) {
                    return { success: false, error: 'Tên chiến dịch mới không được để trống' };
                }
                targetCampaignId = db.saveCRMCampaign({
                    ...newCampaign,
            ...
            if (!targetCampaignId) {
                return { success: false, error: 'Chưa chọn chiến dịch đích' };
            }

            const result = db.addCampaignContacts(targetCampaignId, zaloId, contacts);
            db.save();

            EventBroadcaster.emit('crm:campaignChanged', { action: 'save', ownerZaloId: zaloId, id: targetCampaignId });
