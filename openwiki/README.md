...
- **🏠 Tự Động Nhận Diện CSDL Cũ & Khắc Phục License Gate**: Kế thừa Deplao-gốc phát hiện `zagi-tool.db` đã có trên máy để kích hoạt bản quyền vĩnh viễn và vào thẳng Dashboard, loại bỏ 100% màn hình License Popup khi cài đè hoặc nâng cấp phiên bản.
...
- **👥 Bộ Chọn Tài Khoản Quét Số Hàng Loạt Trực Quan**: Multi-select Checkbox mặc định chọn sẵn 100% tài khoản active, hỗ trợ 1-click bỏ chọn và phím tắt `[ Chọn tất cả ]` / `[ Chỉ chọn 1 TK ]` để quét độc quyền hoặc song song.
- **🏷️ Chuẩn Hóa Biến Tên Thật & Xưng Hô Tự Nhiên**: Bảo toàn biệt danh Zalo `[Tên lô] - [Tên Zalo] - [SĐT]` khi chọn *"TÊN ZALO SAU KHI GỬI: Không đổi"*; Biến `{name}` và `{real_name}` ưu tiên Tên thật từ Excel / nhập tay ➔ fallback về Tên Zalo gốc sạch sẽ, kèm xưng hô `{salutation}` & `{tu_xung}` thông minh theo ngữ cảnh tiếng Việt.

- 2026-08-08: v3.1.7 — Tự động nhận diện CSDL cũ xóa bỏ License Popup; Giải mã Cookie 4 lớp cho Zalo Accounts tự động Online; Đăng ký Core IPC sớm khắc phục lỗi `getPipelineStages` & tự gieo 6 cột phễu Kanban mẫu; Nâng cấp Bộ chọn Tài khoản Quét số hàng loạt dạng Multi-select Checkbox mặc định chọn tất cả kèm phím tắt 1 chạm; Tách biệt Biệt danh Gợi nhớ Zalo và Chuẩn hóa biến Tên thật `{real_name}`, `{name}` cùng bộ quy tắc `{salutation}` & `{tu_xung}` thông minh theo ngữ cảnh tiếng Việt.
