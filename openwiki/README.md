...
## Release Highlights (v3.0.7)

- **👑 Role Isolation & License Gate**: Onboarding chọn Boss/Employee, License Gate 2 lớp bảo vệ Boss Mode, ẩn Workspace Boss & CSDL trên giao diện Nhân viên.
- **📊 Gradient Campaign Report**: Báo cáo chiến dịch với 4 thẻ Gradient sống động, bộ lọc khoảng thời gian quét Zalo linh hoạt.
- **📋 Dual-Option Campaign Clone**: Nhân bản chiến dịch CÓ hoặc KHÔNG kèm danh sách khách hàng.
- **🔒 Security & Clean Code**: Khắc phục 100% cảnh báo bảo mật scanner, gom nhóm `isEmployeeMode()` và `ipcHandle()` về `proxyHelper.ts`, xóa bỏ hàm trùng lặp.

- 2026-08-04: v3.1.6 — Chuẩn hóa Bộ Từ điển Mã Lỗi Zalo (`ZaloErrorDictionary.ts`): Bắt chính xác mã lỗi Zalo API (`-216`, `108`, `127`, `300`, `50012`, `-201`, `5001`), tự động tạm dừng chiến dịch (`paused`) khi gặp lỗi do hạn ngạch nick gửi để bảo vệ an toàn tài khoản; Nâng cấp hàng đợi Quét số hàng loạt 1 lô duy nhất (Lưu nháp, Chạy ngay, Ưu tiên chạy) kèm Banner cảnh báo nổi phía trên nút bấm Khởi tạo; Tự động migrations cột `is_deleted`/`deleted_at` giải quyết sự cố lưu chiến dịch trên Windows.
