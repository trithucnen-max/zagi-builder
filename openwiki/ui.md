...
4. **Switch to BOSS Mode**: Khi cần chuyển từ Chế độ Nhân viên trở lại Chế độ BOSS (Sếp / Chủ shop), người dùng nhấp vào trạng thái kết nối `Connected - [Tên Nhân Viên]` trên thanh TopBar Header và chọn **👑 Chuyển sang máy BOSS (Sếp)**. Ứng dụng sẽ ngắt kết nối session nhân viên, tự động switch active workspace về `'default'` và chuyển `empMode` về `'boss'` ngay lập tức.
