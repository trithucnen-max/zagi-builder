...
class WorkspaceManager {
    ...
    private migrateFromLegacy(): void {
        ...
        let zagiConfigPath = path.join(this.userDataPath, 'zagi-config.json');
        if (!fs.existsSync(zagiConfigPath)) {
            const legacyConfig = path.join(this.userDataPath, 'deplao-config.json');
            if (fs.existsSync(legacyConfig)) zagiConfigPath = legacyConfig;
        }
        if (fs.existsSync(zagiConfigPath)) {
            try {
