  ...
  interface Window {
    ...
    electronAPI: {
      ...
      db: {
        ...
        getPipelineStages: () => Promise<{ success: boolean; stages: any[]; error?: string }>;
      ...
      crm: {
        ...
        getPhoneScanBatches: (params?: { accountIds?: string[] }) => Promise<{ success: boolean; batches?: any[]; error?: string }>;
        getPhoneScanOverallStats: (params?: { timeRange?: 'all' | 'today' | 'this_week' | 'this_month' | 'custom'; startDate?: string; endDate?: string; accountIds?: string[] }) => Promise<{ success: boolean; stats?: { total: number; scanned: number; found: number; notFound: number; error: number; pending: number; startTimestamp?: number; endTimestamp?: number }; error?: string }>;
        getPhoneScanItems: (params: { batchId: number; limit?: number; offset?: number; status?: string; scannedByAccountId?: string }) => Promise<{ success: boolean; items?: any[]; total?: number; error?: string }>;
