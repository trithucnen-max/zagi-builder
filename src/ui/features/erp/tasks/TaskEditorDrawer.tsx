...
export default function TaskEditorDrawer({ taskId, defaultStatus = 'todo', projectId, onClose, onSaved }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-[1100px] bg-gray-950 border-l border-gray-800 h-full overflow-hidden shadow-2xl flex flex-col text-gray-200 animate-slide-in" onClick={event => event.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800/80 flex-shrink-0 bg-gray-950/20">
          <div className="flex items-center gap-3">
            <h3 className="text-sm font-semibold text-gray-100">{taskId ? 'Chi tiết nhiệm vụ' : 'Tạo nhiệm vụ mới'}</h3>
            {taskId && (
              <button type="button" onClick={handleToggleCompleted} className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 transition-all border ${task?.status === 'done' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-gray-800 border-gray-700 text-gray-400 hover:text-white hover:border-gray-500'}`}>
        ...
        {loading ? (
          ...
          <div className="flex-1 overflow-hidden bg-gray-950">
            <div className="h-full grid grid-cols-1 lg:grid-cols-5 overflow-hidden">
              ...
              <div className="lg:col-span-3 overflow-y-auto p-6 space-y-6 erp-scroll-y h-full border-r border-gray-800/60 bg-gray-950">
                ...
                <div className="space-y-3.5 border-t border-gray-800/60 pt-5">
                  <div className="flex items-center justify-between">
                    ...
                    {checklistTotal > 0 && (
                      <div className="w-32 flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-gray-900 rounded-full overflow-hidden"><div className="h-full bg-blue-500 transition-all duration-300" style={{ width: `${checklistPercent}%` }} /></div>
                  ...
                  <div className="space-y-1.5">
                    {checklists.map(item => (
                      <div key={item.id} className="group flex items-center gap-3 px-3 py-2 rounded-xl bg-gray-900/20 border border-gray-850 hover:border-gray-700 transition-all">
                        ...
                        <span className={`text-xs flex-1 min-w-0 truncate ${item.done ? 'line-through text-gray-500 opacity-50' : 'text-gray-200'}`}>{item.content}</span>
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <select value={item.assignee_id || ''} onChange={e => handleChecklistAssigneeChange(item.id, e.target.value)} className="bg-transparent border-0 text-[10px] text-gray-400 hover:text-gray-200 cursor-pointer focus:outline-none w-20 truncate">
                            <option value="" className="bg-gray-900 text-gray-500">Chưa gán</option>
                            {assigneeOptions.map(opt => <option key={opt.employee_id} value={opt.employee_id} className="bg-gray-900 text-gray-300">{opt.display_name}</option>)}
                          </select>
                          <input type="date" value={item.due_date ? new Date(item.due_date).toISOString().split('T')[0] : ''} onChange={e => handleChecklistDueDateChange(item.id, e.target.value)} className="bg-transparent border-0 text-[10px] text-gray-400 hover:text-gray-200 cursor-pointer focus:outline-none w-24 text-right" />
                    ...
                    <div className="flex items-center gap-3 px-3 py-2 rounded-xl border border-dashed border-gray-800 bg-gray-950/5">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <input type="date" value={subtaskDueDate} onChange={e => setSubtaskDueDate(e.target.value)} className="bg-transparent border-0 text-[10px] text-gray-500 hover:text-gray-300 cursor-pointer focus:outline-none w-24 text-right" />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="lg:col-span-2 overflow-y-auto p-6 space-y-6 erp-scroll-y h-full bg-gray-900/30 border-l border-gray-800/60">
                <Field label="Dự án">
                  <select value={form.project_id} onChange={event => setForm(current => ({ ...current, project_id: event.target.value }))} className="task-editor-select w-full bg-gray-950 border border-gray-800 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-blue-500 text-gray-200">
                    <option value="" className="bg-gray-900">Không thuộc dự án</option>
                    {projects.map(project => <option key={project.id} value={project.id} className="bg-gray-900">{project.name}</option>)}
                  </select>
                </Field>
                <Field label="Trạng thái">
                  <select value={form.status} onChange={event => setForm(current => ({ ...current, status: event.target.value as ErpTaskStatus }))} className="task-editor-select w-full bg-gray-950 border border-gray-800 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-blue-500 text-gray-200">
                    {STATUS_OPTS.map(option => <option key={option.value} value={option.value} className="bg-gray-900">{option.label}</option>)}
                  </select>
                </Field>
                <Field label="Độ ưu tiên">
                  <select value={form.priority} onChange={event => setForm(current => ({ ...current, priority: event.target.value as ErpTaskPriority }))} className="task-editor-select w-full bg-gray-950 border border-gray-800 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-blue-500 text-gray-200">
                    {PRIORITY_OPTS.map(option => <option key={option.value} value={option.value} className="bg-gray-900">{option.label}</option>)}
                  </select>
                </Field>
                <Field label="Hạn hoàn thành">
                  <input type="datetime-local" value={form.due_date} onChange={event => setForm(current => ({ ...current, due_date: event.target.value }))} className="task-editor-select w-full bg-gray-950 border border-gray-800 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-blue-500 text-gray-200" />
                </Field>
