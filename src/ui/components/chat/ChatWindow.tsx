          ...
          onForward={async (threadId, threadType) => {
            ...
            try {
              if (msgIsFile) {
                ...
                const res = await ipc.zalo?.sendFile({ auth, filePath: localPath, threadId, type: threadType });
                if (res && !res.success) {
                  throw new Error(res.error || 'Gửi file thất bại');
                }
              } else if (msgIsImage) {
                ...
                const res = await ipc.zalo?.sendImage({ auth, filePath: localPath, threadId, type: threadType, message: '' });
                if (res && !res.success) {
                  throw new Error(res.error || 'Gửi ảnh thất bại');
                }
              } else {
                // Text message: use forwardMessage API with fallback to sendMessage
                ...
                let res = await ipc.zalo?.forwardMessage({ auth, message: messagePayload, threadIds: [threadId], type: threadType });
                if (res && !res.success) {
                  // Fallback: send as regular text message
                  res = await ipc.zalo?.sendMessage({ auth, threadId, type: threadType, message: textContent });
                  if (res && !res.success) {
