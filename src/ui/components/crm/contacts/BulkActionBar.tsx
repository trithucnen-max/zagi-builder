  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-gray-800 border border-gray-600 rounded-2xl px-5 py-2.5 shadow-2xl">
      ...
      <div ref={moreRef} className="relative flex-shrink-0">
        ...
        {showMore && (
          <div className="absolute bottom-full mb-2 right-0 bg-gray-800 border border-gray-600 rounded-xl shadow-2xl py-1 w-52 z-50">
            {/* Gán xưng hô hàng loạt */}
            {onBulkSalutation && (
              <button
                type="button"
                onClick={() => { setShowMore(false); onBulkSalutation(); }}
                className="w-full flex items-center gap-2.5 px-4 py-2.5 text-xs text-blue-300 hover:bg-gray-700 hover:text-white transition-colors text-left font-medium cursor-pointer">
            ...
            {onDeleteSelected && (
              <>
                ...
                <button
                  type="button"
                  onClick={() => { setShowMore(false); onDeleteSelected(); }}
                  className="w-full flex items-center gap-2.5 px-4 py-2.5 text-xs text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-colors text-left font-semibold cursor-pointer">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="flex-shrink-0">
