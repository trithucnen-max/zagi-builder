        ...
        <div className="flex-1 overflow-y-auto bg-gray-50/60 p-6">
          <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="text-center py-2 space-y-1">
              ...
              <p className="text-xs text-gray-500">Tiếp cận hàng nghìn khách hàng tiềm năng từ các nhóm Zalo chất lượng cao</p>
            </div>

            {/* 4 Feature cards - White background, rounded-2xl, soft shadow */}
            <div className="grid grid-cols-2 gap-3.5">
              {[
                {
                  icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-blue-600"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>,
                  title: 'Nhóm ẩn thành viên',
                  desc: 'Admin ẩn danh sách thành viên - hệ thống vẫn tự động bóc tách đầy đủ'
                },
                {
                  icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-indigo-600"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
                  title: 'Nhóm chờ duyệt',
                  desc: 'Chưa được duyệt vào nhóm vẫn quét được danh sách thành viên tức thì'
                },
                {
                  icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-emerald-600"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>,
                  title: 'Tốc độ siêu nhanh',
                  desc: 'Quét và bóc tách dữ liệu 1.000+ thành viên chỉ trong vài giây'
                },
                {
                  icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-amber-600"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>,
                  title: 'Kết quả chi tiết',
                  desc: 'Tự động phân định Trưởng/Phó nhóm, lưu CSDL CRM để sẵn sàng chạy chiến dịch'
                },
              ].map((f, i) => (
                <div key={i} className="bg-white border border-gray-200/80 rounded-2xl p-4 flex gap-3.5 shadow-sm hover:shadow transition-shadow">
                  <div className="w-10 h-10 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center flex-shrink-0">{f.icon}</div>
                  <div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 text-[11px] font-semibold transition-colors flex items-center gap-1">
                    {premiumLoading ? <>{SpinIcon} Cập nhật...</> : <>{RefreshIcon} Cập nhật</>}
                  </button>
            ...
            {/* How it works */}
            <div className="bg-white border border-gray-200/80 rounded-2xl p-5 shadow-sm space-y-3">
              <p className="text-xs font-bold text-gray-900 uppercase tracking-wider">Cách sử dụng 4 bước</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {[
                  { step: '1', text: 'Dán link nhóm Zalo (hoặc Group ID) vào ô bên trên' },
                  { step: '2', text: 'Nhấn "Tham gia" nếu tài khoản chưa là thành viên' },
                  { step: '3', text: 'Nhấn "Quét" — kết quả sẽ tự động lưu vào CSDL' },
                  { step: '4', text: 'Chọn thành viên ➔ thêm vào chiến dịch gửi tin, kết bạn...' },
                ].map(s => (
                  <div key={s.step} className="flex items-start gap-3 p-2.5 rounded-xl bg-gray-50/70 border border-gray-100">
                    <span className="w-6 h-6 rounded-full bg-blue-100 text-blue-700 font-bold text-xs flex items-center justify-center flex-shrink-0 mt-0.5">{s.step}</span>
