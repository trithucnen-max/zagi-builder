...
async function fetchGroupInfoAndMembers(zaloId: string, groupId: string, forceNotifUpdate = false): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      dbMembers = membersRes?.members || [];
      hasMembers = dbMembers.length > 0;
    } catch {}

    // Nếu đã có đầy đủ thông tin và không cần force update → bỏ qua
    if (hasRealName && hasMembers && !forceNotifUpdate) {
      // Vẫn cần đồng bộ DB members lên groupInfoCache nếu trong cache chưa có members
      const cached = useAppStore.getState().groupInfoCache?.[zaloId]?.[groupId];
      if (!cached || !cached.members || cached.members.length === 0) {
        const contact = useChatStore.getState().contacts[zaloId]?.find(c => c.contact_id === groupId);
