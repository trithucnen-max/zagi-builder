...
* **Tổ chức nội dung:** 
  ...
  * Nội dung chi tiết phong phú, hỗ trợ render các thẻ alert định dạng GitHub (Note/Warning) giúp làm nổi bật các lưu ý kỹ thuật (ví dụ: quét nhóm ẩn lockViewMember, gửi nhiều file, v.v.).### 6.3. Quy chuẩn Node cấu hình Workflow (Workflow Node Config Panel)
* **Cơ chế cuộn (Scrollbar Layout):** Khung chứa form cấu hình node bắt buộc phải có thuộc tính `min-h-0` kết hợp `overflow-y-auto` để đảm bảo khi danh sách tùy chọn cấu hình quá dài, Flexbox item không bị giãn nở vô tận ra ngoài viewport, mà sẽ tự động kích hoạt thanh cuộn dọc (scrollbar) mượt mà.
* **Tính năng Xem trước (Preview Modal):**
  * Đối với các node truy vấn dữ liệu như `crm.getContacts`, bắt buộc cung cấp nút xem trước danh sách đối tượng lọc được ở cuối form.
  * Modal xem trước phải sử dụng avatar nhóm dạng composite grid (`GroupAvatar` của Zagi) để thể hiện đúng ảnh đại diện của nhóm chat và cá nhân.
  * Các trường thông tin phải được việt hóa kèm emoji icon sinh động (Giới tính: `♂ Nam`, `♀ Nữ`, `👥 Nhóm`; Mối quan hệ: `👥 Bạn bè`, `👤 Chưa kết bạn`; Kênh: `💬 Zalo`, `📘 Facebook`).
