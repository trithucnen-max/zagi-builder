...
#### 🚀 v3.2.0 — Tạo Bình Chọn Zalo Đầy Đủ 100%, Đồng Bộ Xưng Hô 2 Chiều & Nâng Cấp Menu Pipeline CRM (Official Release)
* **Tính năng mới & Sửa lỗi nổi bật:**
  * **🗳️ Nâng Cấp Toàn Diện Tính Năng Tạo Bình Chọn Nhóm Chuẩn 100% Theo Zalo (`ChatWindow.tsx`, `ZaloService.ts`)**:
    * **Giao diện 2 cột chuẩn Zalo**: Thiết kế tối ưu trải nghiệm: Cột trái chứa Chủ đề bình chọn (textarea kèm bộ đếm `0/200` ký tự) và Danh sách lựa chọn (tối thiểu 2, tối đa 20) hỗ trợ thêm/xóa nhanh từng mục; Cột phải chứa Thời hạn bình chọn và toàn bộ Thiết lập nâng cao.
    * **Thời hạn bình chọn thông minh**: Hộp chọn ngày/giờ có icon lịch 📅 và nút xóa nhanh để đưa về trạng thái "Không thời hạn".
    * **Thiết lập nâng cao đầy đủ**:
      ...
      * 🛈 **Chọn nhiều phương án (`allowMultiChoices`)**: Cho phép thành viên chọn nhiều phương án, kèm icon `?` tooltip giải thích.
      * 🛈 **Có thể thêm phương án (`allowAddNewOption`)**: Cho phép thành viên tự thêm lựa chọn mới vào cuộc bình chọn.
    * **Bình chọn ẩn danh**:
      * 🛈 **Ẩn kết quả khi chưa bình chọn (`hideVotePreview`)**: Thành viên phải bỏ phiếu trước mới xem được kết quả hiện tại.
      * 🛈 **Ẩn người bình chọn (`isAnonymous`)**: Ẩn danh tính của người đã tham gia bỏ phiếu.
    * **Lưu thiết lập mặc định ⚙️**: Nút bánh răng ở góc dưới bên trái cho phép lưu toàn bộ các tùy chọn bật/tắt yêu thích vào `localStorage` để tự động áp dụng cho các lần tạo sau.
...
* **Tính năng mới & Sửa lỗi nổi bật:**
  ...
  * **🔀 Đột Phá Option C: Fallback Single Mode Bền Bỉ (`PhoneScanService.ts`)**:
    ...
    * Khi kết quả quét gộp không thấy SĐT trong map phản hồi, tự động kích hoạt fallback `findUser` đơn lẻ ngay lập tức trước khi kết luận không tìm thấy.
  * **⏳ Cơ Chế Smart Cooldown 3 Phút & Ngưỡng Lỗi Liên Tiếp 3 Lần (Không Hủy Cả Lô Vì 1 Số)**:
    * Bổ sung bộ đếm `consecutiveSingleRateLimitCount` và thời gian nghỉ `accountCooldownUntil` (3 phút).
    * **Lỗi 1-2 lần đơn lẻ trên số điện thoại**: Tự động rollback SĐT đó về trạng thái `Chờ quét (pending)` và đưa nick vào trạng thái nghỉ thông minh 3 phút để hạ nhiệt, cho phép scheduler tiếp tục quét các nick khác hoặc quét các số còn lại sau khi hết thời gian nghỉ.
    * **Chỉ khi 3 số điện thoại khác nhau liên tiếp đều trả về `-216`**: Hệ thống mới kết luận tài khoản đã thực sự cạn kiệt hạn ngạch tìm kiếm của Zalo và mới tạm dừng tài khoản.
  * **⚡ Bộ Phím Điều Khiển Quét Tức Thì Trong Báo Cáo Phóng To (`PhoneScanPanel.tsx`)**:
    * **`⚡ Tiếp tục quét Single Mode (findUser)`**: Unpause lô ngay lập tức, xóa cooldown tài khoản, chuyển sang Single Mode và kích hoạt quét ngay.
    * **`🔄 Quét lại các số Lỗi (Mã -216)`**: Reset toàn bộ các số có trạng thái `error` về lại `pending`, xóa lỗi của lô và tiếp tục quét an toàn bằng Single Mode.
    * **`🔄 Đổi Nick quét tiếp`**: Hiển thị danh sách các tài khoản Zalo đang kết nối ổn định (`Gohr Platform`, `Nguyen Tat Huu`...) để bấm 1 chạm chuyển toàn bộ các số còn lại sang nick mới và quét tiếp ngay lập tức.
  * **🎯 Chuẩn Hóa Bộ Lọc Thẻ Quota & Tránh Chọn Nick Mất Kết Nối**:
    * Thẻ Hạn ngạch & Phân bổ (`📊 Trạng thái Hạn ngạch & Phân bổ`) trong Báo cáo phóng to chỉ hiển thị chính xác các tài khoản Zalo được gán trong Lô kèm tỷ lệ `%` phân bổ tương ứng.
    * Trong Modal Tạo Lô Quét: Tự động vô hiệu hóa các tài khoản Zalo đang bị mất kết nối và gắn huy hiệu đỏ `⚠️ Mất kết nối` để người dùng không chọn nhầm tài khoản chết.
...
| :--- | :--- | :--- | :--- |
| **v3.1.2** | 03/08/2026 | Major Feature | **Hệ Thống Hàng Đợi Thực Thi Chiến Dịch CRM (FIFO + Priority Queue):** Thiết lập quy tắc 1 thời điểm chỉ chạy 1 chiến dịch / 1 tài khoản Zalo. Chiến dịch thứ 2 khởi chạy sẽ tự động chuyển sang trạng thái Hàng đợi (`status = 'queued'`). Hỗ trợ cài đặt Mức ưu tiên (`priority = 'high' | 'normal'`). Tự động đôn chiến dịch hàng đợi (Ưu tiên Cao xếp trước, sau đó theo FIFO thời gian `queued_at`) khi chiến dịch đang chạy hoàn thành hoặc tạm dừng. **Phân Biệt 3 Lý Do Tạm Dừng Chiến Dịch (`pause_reason`):** Phân biệt rõ `user_manual` (Tạm dừng thủ công do người dùng bấm - KHÔNG tự chạy lại), `daily_quota` (Tạm dừng do hết quota an toàn - Tự chạy lại ngày mới 00:00 ICT), và `quiet_hours` (Tạm dừng giờ nghỉ đêm - Tự chạy lại sau 07:00). **Nâng Cấp Checkbox 3 Trạng Thái Gán/Gỡ Nhãn Hàng Loạt:** Cập nhật `UnifiedLabelPickerModal.tsx` hỗ trợ `[✓]` (Gán thêm), `[-]` (Giữ nguyên nhãn cũ), `[ ]` (Gỡ bỏ) tránh ghi đè sai nhãn hàng loạt. **Khắc phục Lỗi Zalo 127 & Làm Sạch Tin Nhắn Bong Bóng:** Tự động phát hiện lỗi Zalo 127 ngắt tự động, làm sạch chuỗi mã hóa `sendBubbleMessage` thành `🔗 [Liên kết chia sẻ]`. **Chuẩn Hóa Dialog Xác Nhận & Banner Hướng Dẫn:** Tự động chuẩn hóa `ConfirmDialog.tsx` hỗ trợ cả tham số string, bổ sung tab lọc `Đang chờ` và Banner hướng dẫn "Thêm đối tượng gửi" khi tạo chiến dịch chưa có liên hệ. **Hệ Thống Định Mức An Toàn Tùy Chọn Theo Tài Khoản Zalo:** Quản lý riêng 2 định mức độc lập (Tin nhắn người lạ/ngày & Lời mời kết bạn/ngày) cho từng nick Zalo với 3 nút chọn nhanh (`🌱 Nick mới 15`, `🌿 Nick thường 30`, `🌳 Nick cũ 50`). |
