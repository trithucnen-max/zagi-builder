...
| :--- | :--- | :--- | :--- |
| **v27.1.7** | 06/2026 | Patch | Thiết kế lại UI/UX (Zalo PC style, Zagi Navy, Purple Ban); macOS Code Signing & Notarization; Nâng cấp Workflow (gửi nhiều ảnh/file, trình chọn ảnh, gửi ngẫu nhiên, sandbox debugger, kịch bản BĐS); Hướng dẫn sử dụng tích hợp Settings; Chuẩn hóa giao diện Brand Logo (SVG trắng trên nền màu gốc) và các tiêu đề danh mục; Sửa lỗi SQLite & đồng bộ thông tin nhóm Zalo; Chuẩn hóa phóng to/thu nhỏ cỡ chữ (CSS Variable) & thống nhất nút bấm. |
...
#### 🍎 v27.1.7 — macOS Code Signing, Sandbox Debugger, Workflow Enhancements, UI/UX & Built-in User Guide
*   **Tính năng mới (New):**
    ...
    *   **macOS Code Signing & Notarization:** Cấu hình bảo mật Hardened Runtime cho macOS thông qua tệp `entitlements.mac.plist` và `entitlements.mac.inherit.plist` cùng khâu ký số tự động.
