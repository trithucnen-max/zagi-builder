...
| :--- | :--- | :--- | :--- |
| **v3.0.6** | 24/07/2026 | Minor | **Quy tắc Đặt tên Gợi nhớ Zalo & Động cơ Normalization SĐT/Thuộc tính:** Thêm 3 quy tắc đặt tên gợi nhớ Zalo (`Không đổi`, `[Tên chiến dịch] - [Tên Zalo] - [SĐT]`, `[Tên Zalo] - [SĐT]`) bảo vệ biệt danh người quen. Tích hợp bóc tách tên gốc `extractCoreZaloName` chống lặp nối biệt danh. **Tái thiết kế Modal Khởi tạo lô quét SĐT & Tải tệp mẫu Excel:** Mở rộng modal lên 1280px x 800px, thêm nút tải file mẫu Excel `.xlsx` 3 cột (`SĐT`, `Giới tính`, `Ngày sinh`) và Dropzone kéo thả file `.xlsx`, `.xls`, `.csv`. Tự động bổ sung số 0 cho SĐT 9 chữ số (`912345678` ➔ `0912345678`), quy đổi giới tính (`nam`, `male`, `1` ➔ `Nam`) và ngày sinh (`15-08-1992` ➔ `15/08/1992`). **Sửa triệt để lỗi màu sắc Giao diện Buổi tối (Dark Mode):** Chuẩn hóa toàn bộ các class CSS Tailwind `gray-850/750/650`, sửa lỗi mảng màu trắng tràn ở Editor và ô tin nhắn `textarea` trên tất cả các chế độ chiến dịch. |
