...
| :--- | :--- | :--- | :--- |
| **v3.0.6** | 24/07/2026 | Minor | **Trình duyệt Web Nhân viên (Employee Web Client) & Progressive Web App (PWA):** Đồng bộ 100% tính năng cho Nhân viên chạy qua Trình duyệt Web (`http://127.0.0.1:27799`). Tích hợp `manifest.json` & Service Worker `sw.js` cho phép cài đặt Zagi Web thành **App Cửa sổ Độc lập** có Icon riêng trên màn hình Desktop/Taskbar mà không cần cài file `.dmg`/`.exe`. **Động cơ Realtime SSE Stream & Hiển thị Tức thì 0.05s:** Đồng bộ tức thì các lệnh gửi tin nhắn văn bản, ảnh từ Thư viện Media, video, tệp tin từ Trình duyệt Web lên khung chat trong 0.05s không cần nhấn F5. **Trình Phục vụ Tệp Media Tốc độ cao Cho Web:** Tự động chuyển đổi các đường dẫn đĩa local (`local-media:///...`) thành HTTP REST URL: `${bossUrl}/api/media/file?path=...` kèm mở rộng handler `GET /api/media/file?path=...` trên máy Boss phục vụ truyền dữ liệu hình ảnh, video với đầy đủ header MIME & CORS. **Quy tắc Đặt tên Gợi nhớ Zalo & Động cơ Normalization SĐT/Thuộc tính:** Thêm 3 quy tắc đặt tên gợi nhớ Zalo (`Không đổi`, `[Tên chiến dịch] - [Tên Zalo] - [SĐT]`, `[Tên Zalo] - [SĐT]`) bảo vệ biệt danh người quen. Tích hợp bóc tách tên gốc `extractCoreZaloName` chống lặp nối biệt danh. **Tái thiết kế Modal Khởi tạo lô quét SĐT & Tải tệp mẫu Excel:** Mở rộng modal lên 1280px x 800px, thêm nút tải file mẫu Excel `.xlsx` 3 cột (`SĐT`, `Giới tính`, `Ngày sinh`) và Dropzone kéo thả file `.xlsx`, `.xls`, `.csv`. Tự động bổ sung số 0 cho SĐT 9 chữ số (`912345678` ➔ `0912345678`), quy đổi giới tính (`nam`, `male`, `1` ➔ `Nam`) và ngày sinh (`15-08-1992` ➔ `15/08/1992`). **Chuẩn hóa & Đồng bộ Trạng thái Bạn bè Zalo (`is_friend`):** Ép gán cờ `is_friend` dựa trên đối chiếu thực tế với bảng `friends` của từng tài khoản Zalo, loại bỏ hoàn toàn biểu tượng dấu tích xanh (`✓`) nhầm lẫn đối với SĐT Quét/Khách lạ. Tích hợp tự động reset cờ `is_friend` rác trong bảng `contacts` khi bấm nút "Dọn dẹp biệt danh dính chéo" trên Modal Rà soát lọc trùng. |
