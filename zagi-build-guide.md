...
flowchart TD
    ...
    subgraph "3. Đóng gói (electron-builder)"
        ...
        H -->|Chạy trên Windows| I["📥 Zagi-Setup-26.4.3.exe"]
        H -->|Chạy trên macOS| J["📥 Zagi-26.4.3-arm64.dmg / .dmg Intel"]
    end
```
...
- `Zagi-26.4.3-arm64.dmg` (Dành cho máy Mac dùng chip Apple Silicon: M1, M2, M3, M4...).
- `Zagi-26.4.3.dmg` (Dành cho máy Mac dùng chip Intel).
- `Zagi-26.4.3-mac.zip` (Bản nén zip chạy trực tiếp không cần cài đặt).
...
- `Zagi-Setup-26.4.3.exe` (File cài đặt chuẩn cho Windows 10/11).
